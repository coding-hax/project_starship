...
# brauchen legitim keinen Dexie-Bump (siehe docs/workflow/ticket-und-tests.md, Issue #59).
set -uo pipefail
...
if [ -n "$MIG" ] && [ -z "$DEXIE" ]; then
  echo "::warning::Server-Migration berührt, aber src/local/dexie.ts trägt keinen neuen db.version()-Bump. Bewusst bestätigen, dass das Client-Schema wirklich unberührt bleibt (docs/workflow/ticket-und-tests.md, Migrationen)."
else
