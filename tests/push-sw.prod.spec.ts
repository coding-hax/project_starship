 ...
 *
 * Drives the actual service worker via `self.__pushTest`/`self.__lastNotificationClick`
...
test('Tippen auf die Benachrichtigung öffnet die App (AC1)', async ({ context }) => {
  ...
  await worker.evaluate(async () => {
    ...
    scope.dispatchEvent(new NotificationEvent('notificationclick', { notification }));
  });
