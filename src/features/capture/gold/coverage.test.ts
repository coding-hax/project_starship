...
it('kein Fall fällt durch die Filter des Gates', () => {
  ...
  const kombi = generateCombinedCases();
  const wtag = generateWeekdayDayPartCases();
  const covered =
    ...
    spoken.filter((c) => ['spoken:kopf:', 'spoken:kopf-datum', 'spoken:aussage', 'spoken:zoegern:', 'spoken:zoegern-aussage', 'spoken:relativsatz'].some((p) => c.id.startsWith(p))).length +
    ...
    kombi.filter((c) => ['kombi:wdh-spanne', 'kombi:kopf-wdh', 'kombi:kopf-datum-spanne', 'kombi:zoegern-aussage-datum', 'kombi:telegramm-spanne', 'kombi:kopf-praeposition'].some((p) => c.id.startsWith(p))).length +
    wtag.filter((c) => c.id.startsWith('wtag')).length;
  expect(covered).toBe(gen.length + hard.length + spoken.length + tele.length + komplex.length + kombi.length + wtag.length);
});
