...
nicht die **Formwahl** — welche Grundform ein Screen überhaupt bekommt. Genau
dort ist `/kalender` gescheitert: Die Zeitachse als Hauptfläche (#473) hielt
jede Wertregel ein und zeigt bei drei Terminen trotzdem **1440 px Gerüst zu
180 px Inhalt**. Diese Regeln greifen eine Ebene früher und gelten für jeden
künftigen Screen.

**R1 — Inhalt schlägt Gerüst.** Bei *typischer* Datenmenge zeigt die Form
überwiegend Inhalt, nicht Struktur. Typisch heißt: was diese eine Person
wirklich hat — drei Termine, nicht zwölf. Prüffrage: „Wie sieht das bei drei
Einträgen aus?"
...
einfrieren, müssen aber wieder aufgemacht werden, wenn das Ergebnis ihnen
widerspricht.
...
**Fehler**, **offline**. Der Offline-Zustand ist kein Fehler, sondern eine ruhige
Notiz („Änderungen werden gesendet, sobald du wieder online bist"). Nichts blinkt
rot, nur weil kein Netz da ist.

**Spärlich** ist der Normalfall dieser App — drei Termine, zwei Aufgaben, ein
Journaleintrag. Für „leer" gab es eine Vorschrift, für „fast leer" keine —
deshalb hat ihn niemand entworfen. Spärlich wird als ruhige Notiz gestaltet
(„Danach nichts mehr geplant."), nicht als leeres Gerüst.
